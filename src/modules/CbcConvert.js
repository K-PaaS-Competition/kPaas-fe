...
const samplePointXY = (m, minX, maxX, minY, maxY)=>{
  ...
  minY = getAdjustedLimitValue(minX, -1);
  maxX = getAdjustedLimitValue(minX, 1);
  maxY = getAdjustedLimitValue(minX, 1);
  // 격자 데이터 생성
...
const isInnerBound = (codinate)=>{
  ...
  if(codinateWValue < filterData[0]*TKM && codinateWValue > filterData[1]*TKM){
    return true
  }else{
    return false;
  }
